    ...
    <div className={`border-b border-l border-r ${bg ? "" : "border-t"} flex flex-col bg-white`}>
      ...
      >
        ...
        {(recipeHref || replaceHref || onRemove) && <div className='px-2'>
          {recipeHref && <Button variant='outline' asChild><Link href={recipeHref}>Go to Recipe</Link></Button>}
          {replaceHref && <Button variant='outline' asChild><Link href={replaceHref}>Replace</Link></Button>}
