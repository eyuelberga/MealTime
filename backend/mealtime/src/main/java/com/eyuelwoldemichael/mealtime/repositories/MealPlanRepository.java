...
import com.eyuelwoldemichael.mealtime.dto.mealplan.response.MealFrequencyByRecipeResponse;
import com.eyuelwoldemichael.mealtime.dto.mealplan.response.MealPlanAggregateByRecipeResponse;
import com.eyuelwoldemichael.mealtime.dto.mealplan.response.MealPlanAggregateResponse;
import com.eyuelwoldemichael.mealtime.dto.mealplan.response.MealFrequencyByMealTypeResponse;
...
public interface MealPlanRepository extends JpaRepository<MealPlan, String> {
  ...
  @Query("Select new com.eyuelwoldemichael.mealtime.dto.mealplan.response.MealPlanAggregateResponse(count(m.id), sum(m.recipe.nutrition.calories),sum(m.recipe.nutrition.totalFat),sum(m.recipe.nutrition.saturatedFat),sum(m.recipe.nutrition.cholesterol),sum(m.recipe.nutrition.sodium),sum(m.recipe.nutrition.carbohydrates),sum(m.recipe.nutrition.fiber),sum(m.recipe.nutrition.sugar),sum(m.recipe.nutrition.protein))  from MealPlan m where m.date between ?1 and ?2 and m.createdBy= ?3")
  MealPlanAggregateResponse nutritionSumDateBetweenAndCreatedBy(Date startDate, Date endDate, String createdBy);

  @Query("Select new com.eyuelwoldemichael.mealtime.dto.mealplan.response.MealFrequencyByMealTypeResponse(count(m.recipe.id), m.mealType) from MealPlan m where m.date between ?1 and ?2 and m.createdBy= ?3 GROUP BY m.mealType")
  List<MealFrequencyByMealTypeResponse> mealFrequencyByMealType(Date startDate, Date endDate, String createdBy);

  @Query("Select new com.eyuelwoldemichael.mealtime.dto.mealplan.response.MealFrequencyByRecipeResponse(count(m.recipe.id), m.recipe.name) from MealPlan m where m.date between ?1 and ?2 and m.createdBy= ?3 GROUP BY m.recipe.name")
  List<MealFrequencyByRecipeResponse> mealFrequencyByRecipe(Date startDate, Date endDate, String createdBy);


  @Query("Select new com.eyuelwoldemichael.mealtime.dto.mealplan.response.MealPlanAggregateByRecipeResponse(m.recipe.name, sum(m.recipe.nutrition.calories),sum(m.recipe.nutrition.totalFat),sum(m.recipe.nutrition.saturatedFat),sum(m.recipe.nutrition.cholesterol),sum(m.recipe.nutrition.sodium),sum(m.recipe.nutrition.carbohydrates),sum(m.recipe.nutrition.fiber),sum(m.recipe.nutrition.sugar),sum(m.recipe.nutrition.protein))  from MealPlan m where m.date between ?1 and ?2 and m.createdBy= ?3 GROUP BY m.recipe.name")
